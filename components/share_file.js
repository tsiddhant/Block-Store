...
try{
  const reAddress=await main.methods.userdetails(enteredadd).call();
    ...
    try {
      ...
      await reobj.methods.addreceivedfile(this.state.ipfsHash,encr,this.state.fname, accounts[0]).send({
      from:accounts[0],
    ...
    await userobj.methods.addsendfile(this.state.ipfsHash,aesencr,this.state.fname, enteredadd).send({
      from:accounts[0],
      file:this.state.ipfsHash,
      aespass:aesencr,
      nm:this.state.fname,
    ...
    return (
      <div className="App">
            <header className="App-header">
              ...
              <input value={this.state.enteredadd} onChange={event=>this.setState({ enteredadd : event.target.value})}></input>
                <div>
                     <input
                      type = "file"
